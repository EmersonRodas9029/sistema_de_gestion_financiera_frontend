      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div whileHover={{ y: -4, scale: 1.02 }} transition={{ type: 'spring', stiffness: 300 }} className="bg-[#101015] rounded-[20px] p-5 border border-white/[0.07] shadow-lg">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="shrink-0 p-3 rounded-xl bg-white/10"><FileText size={24} className="text-[#8A5CF6]" /></div>
          </div>
        </motion.div>
                    ...
                    >
                      <div className="flex items-center justify-between flex-wrap gap-x-2 gap-y-1 mb-2">
                        <div className="flex items-center gap-2">
                          <div className="p-1.5 rounded-lg bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B] bg-opacity-20 w-6 h-6 flex items-center justify-center text-white text-xs font-bold">
                            {index + 1}
                          </div>
        ...
        >
          <div>
            ...
            <select value={selectedTipo} onChange={(e) => setSelectedTipo(e.target.value as 'todos' | TipoReporte)}
              ...
              <option value="todos" style={{ backgroundColor: '#08080B', color: 'white' }}>Todos los tipos</option>
              {(Object.keys(TIPO_LABEL) as TipoReporte[]).map(tipo => (
                <option key={tipo} value={tipo} style={{ backgroundColor: '#08080B', color: 'white' }}>{TIPO_LABEL[tipo]}</option>
              ))}
            </select>
          </div>
        </SearchFilterBar>
                  ...
                  {paginatedReports.map((report, index) => (
                    <motion.tr key={report.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: index * 0.03 }} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          <FileText size={14} className="text-[#8A5CF6]" />
            ...
            <AnimatePresence>
              {paginatedReports.map((report, index) => (
                <motion.div
                  ...
                  className="relative overflow-hidden bg-gradient-to-br from-white/5 to-white/0 rounded-xl p-4 border border-white/10 hover:border-[#8A5CF6]/50 transition-all duration-300"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-white/10 shadow-lg">
                        <FileText size={16} className="text-[#8A5CF6]" />
                      </div>
                      <div>
        ...
        <ModalOverlay isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Generar Reporte" subtitle="Crea un nuevo reporte financiero" icon={<FileText size={20} className="text-white" />} maxWidth="max-w-2xl">
          <form onSubmit={(e) => { e.preventDefault(); handleCreateReport(); }} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="Ej: Reporte de gastos - Junio" required />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="text" value={formData.rutaArchivo} onChange={(e) => setFormData({ ...formData, rutaArchivo: e.target.value })} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="/reportes/archivo.pdf" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
            ...
            <div>
              <label className="text-white/60 text-sm mb-1.5 flex items-center gap-2">
                ...
                {isAutoFilling && <span className="flex items-center gap-1 text-[#8A5CF6] text-xs"><Loader2 size={12} className="animate-spin" /> Calculando datos reales...</span>}
              ...
              <textarea value={formData.contenido} onChange={(e) => setFormData({ ...formData, contenido: e.target.value })} rows={3} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder='{"total": 1500.00, "categorias": {...}}' />
            </div>
        ...
        <ModalOverlay isOpen={showEditModal && !!selectedReport} onClose={() => setShowEditModal(false)} title="Editar Reporte" subtitle="Modifica los datos del reporte" icon={<Edit size={20} className="text-white" />} maxWidth="max-w-2xl">
          <form onSubmit={(e) => { e.preventDefault(); handleUpdateReport(); }} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-white/60 text-sm mb-1.5 block">Ruta de archivo (opcional)</label>
                <input type="text" value={formData.rutaArchivo} onChange={(e) => setFormData({ ...formData, rutaArchivo: e.target.value })} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
            ...
            <div>
              ...
              <textarea value={formData.contenido} onChange={(e) => setFormData({ ...formData, contenido: e.target.value })} rows={3} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-[#8A5CF6] transition-all" />
            </div>
            <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
            ...
            <div className="space-y-5">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDownloadPdf(selectedReport)} disabled={downloadingId === selectedReport.id}
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B] text-white rounded-lg hover:opacity-90 transition-all font-medium disabled:opacity-50">
      ...
      <AnimatePresence>
        {showDeleteModal && selectedReport && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-[#08080B] rounded-xl border border-white/10 max-w-md w-full shadow-2xl">
