      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div whileHover={{ y: -4, scale: 1.02 }} transition={{ type: 'spring', stiffness: 300 }} className="bg-[#101015] rounded-[20px] p-5 border border-white/[0.07] shadow-lg">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="shrink-0 p-3 rounded-xl bg-white/10"><Target size={24} className="text-[#8A5CF6]" /></div>
          </div>
          <div className="w-full h-2 bg-white/10 rounded-full mt-3 overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${overallProgress}%` }} transition={{ duration: 1 }} className="h-full bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B] rounded-full" />
          </div>
        </motion.div>
              ...
              {topGoals.map((goal) => (
                <motion.div key={goal.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-all duration-300">
                  ...
                  <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${progressOf(goal)}%` }} transition={{ duration: 0.8 }} className="h-full rounded-full bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B]" />
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={selectedEstado} onChange={(e) => setSelectedEstado(e.target.value as Estado)} className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] text-sm" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)', color: 'white' }}>
                <option value="todas" style={{ backgroundColor: '#08080B', color: 'white' }}>Todos los estados</option>
                <option value="activa" style={{ backgroundColor: '#08080B', color: 'white' }}>Activas</option>
                <option value="completada" style={{ backgroundColor: '#08080B', color: 'white' }}>Completadas</option>
                <option value="inactiva" style={{ backgroundColor: '#08080B', color: 'white' }}>Inactivas</option>
              </select>
            </div>
            <div>
              <label className="text-white/60 text-xs mb-1 block">Prioridad</label>
              <select value={selectedPrioridad} onChange={(e) => setSelectedPrioridad(e.target.value as 'todas' | Prioridad)} className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] text-sm" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)', color: 'white' }}>
                <option value="todas" style={{ backgroundColor: '#08080B', color: 'white' }}>Todas las prioridades</option>
                <option value="ALTA" style={{ backgroundColor: '#08080B', color: 'white' }}>Alta</option>
                <option value="MEDIA" style={{ backgroundColor: '#08080B', color: 'white' }}>Media</option>
                <option value="BAJA" style={{ backgroundColor: '#08080B', color: 'white' }}>Baja</option>
              </select>
            </div>
          </div>
        </SearchFilterBar>
                    ...
                    <motion.tr key={goal.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: index * 0.03 }} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          <Target size={14} className="text-[#8A5CF6]" />
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2 w-32">
                          <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B]" style={{ width: `${progressOf(goal)}%` }} />
                          </div>
              ...
              {paginatedGoals.map((goal, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`relative overflow-hidden bg-gradient-to-br from-white/5 to-white/0 rounded-xl p-4 border transition-all duration-300 ${isAtRisk ? 'border-red-500/50' : estado === 'completada' ? 'border-blue-500/50' : 'border-white/10 hover:border-[#8A5CF6]/50'}`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-xl bg-gradient-to-r from-[#8A5CF6]/20 to-[#F26D5B]/20">
                          <Target size={16} className="text-[#8A5CF6]" />
                        </div>
                        <div>
                    ...
                    <div className="space-y-2 mb-3">
                      ...
                      <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.8 }} className={`h-full rounded-full ${progress === 100 ? 'bg-green-500' : isAtRisk ? 'bg-red-500' : 'bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B]'}`} />
                      </div>
                      <div className="flex items-center justify-between text-sm">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleCreateGoal(); }} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="Ej: Fondo de emergencia" required />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min="0.01" value={formData.montoObjetivo} onChange={(e) => setFormData({ ...formData, montoObjetivo: e.target.value })} className="w-full pl-10 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="0.00" required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <DollarSign size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white/40" />
                  <input type="number" step="0.01" min="0" value={formData.montoActual} onChange={(e) => setFormData({ ...formData, montoActual: e.target.value })} className="w-full pl-10 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="0.00" />
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <textarea value={formData.descripcion} onChange={(e) => setFormData({ ...formData, descripcion: e.target.value })} rows={2} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" placeholder="Descripción opcional..." />
            </div>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={formData.activa} onChange={(e) => setFormData({ ...formData, activa: e.target.checked })} className="w-4 h-4 rounded border-gray-300 text-[#8A5CF6] focus:ring-[#8A5CF6] bg-white/5" />
        ...
        <ModalOverlay isOpen={showEditModal && !!selectedGoal} onClose={() => setShowEditModal(false)} title="Editar Meta" subtitle="Modifica los datos de la meta" icon={<Edit size={20} className="text-white" />} maxWidth="max-w-2xl">
          <form onSubmit={(e) => { e.preventDefault(); handleUpdateGoal(); }} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" required />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-white/60 text-sm mb-1.5 block">Monto objetivo</label>
                <div className="relative">
                  <DollarSign size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white/40" />
                  <input type="number" step="0.01" min="0.01" value={formData.montoObjetivo} onChange={(e) => setFormData({ ...formData, montoObjetivo: e.target.value })} className="w-full pl-10 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" required />
                </div>
              </div>
              <div>
                <label className="text-white/60 text-sm mb-1.5 block">Monto actual</label>
                <div className="relative">
                  <DollarSign size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white/40" />
                  <input type="number" step="0.01" min="0" value={formData.montoActual} onChange={(e) => setFormData({ ...formData, montoActual: e.target.value })} className="w-full pl-10 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" />
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-white/60 text-sm mb-1.5 block">Descripción</label>
              <textarea value={formData.descripcion} onChange={(e) => setFormData({ ...formData, descripcion: e.target.value })} rows={2} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#8A5CF6] transition-all" />
            </div>
            <div className="flex items-center gap-6">
              ...
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.completada} onChange={(e) => setFormData({ ...formData, completada: e.target.checked })} className="w-4 h-4 rounded border-gray-300 text-[#8A5CF6] focus:ring-[#8A5CF6] bg-white/5" />
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="px-4 py-2 bg-gradient-to-r from-[#8A5CF6] to-[#F26D5B] text-white rounded-lg hover:opacity-90 transition-all font-medium shadow-lg">
                Guardar Cambios
              </motion.button>
            </div>
          </form>
